//! Extension robustness fuzzer for malformed/edge padding extension payloads.
//!
//! Input layout:
//! - byte 0: requested frame count (mapped to 1..=48)
//! - byte 1..: extension payload and optional packet bytes
#![no_main]
...
fuzz_target!(|data: &[u8]| {
    ...
    if let Ok(total) =
        opus_packet_extensions_count_ext(payload, &mut per_frame[..nb_frames as usize], nb_frames)
    {
