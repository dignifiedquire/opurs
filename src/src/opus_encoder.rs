...
pub unsafe fn opus_encoder_get_size(channels: i32) -> i32 {
    ...
    let ret = silk_Get_Encoder_Size(&mut silkEncSizeBytes);
    if ret != 0 {
        return 0;
    }
    let silkEncSizeBytes = align(silkEncSizeBytes);
    let celtEncSizeBytes = celt_encoder_get_size(channels);
...
pub unsafe fn opus_encoder_init(
    ...
    let silk_enc = (st as *mut i8).offset((*st).silk_enc_offset as isize) as *mut core::ffi::c_void;
    let celt_enc = (st as *mut i8).offset((*st).celt_enc_offset as isize) as *mut OpusCustomEncoder;
    (*st).channels = channels;
    (*st).stream_channels = (*st).channels;
    (*st).Fs = Fs;
    (*st).arch = opus_select_arch();
    let ret = silk_InitEncoder(silk_enc, (*st).arch, &mut (*st).silk_mode);
    if ret != 0 {
    ...
    let err = celt_encoder_init(celt_enc, Fs, channels, (*st).arch);
    if err != OPUS_OK {
        return OPUS_INTERNAL_ERROR;
    }
...
unsafe fn dc_reject(
    ...
    if channels == 2 {
        let mut m0 = *hp_mem.offset(0 as isize);
        let mut m2 = *hp_mem.offset(2 as isize);
        for i in 0..len {
            let x0 = *in_0.offset((2 * i + 0) as isize);
            let x1 = *in_0.offset((2 * i + 1) as isize);
            let out0 = x0 - m0;
            let out1 = x1 - m2;
        ...
        *hp_mem.offset(2 as isize) = m2;
        ...
        let mut m0_0 = *hp_mem.offset(0 as isize);
        for i in 0..len {
            let x = *in_0.offset(i as isize);
            let y = x - m0_0;
...
unsafe fn stereo_fade(
    ...
    let mut i = 0;
    while i < overlap {
        let w = *window.offset((i * inc) as isize) * *window.offset((i * inc) as isize);
...
unsafe fn gain_fade(
    ...
    if channels == 1 {
        ...
        for i in 0..overlap {
            let w_0 = *window.offset((i * inc) as isize) * *window.offset((i * inc) as isize);
...
pub unsafe fn opus_encoder_create(
    ...
    let mut st = malloc(opus_encoder_get_size(channels) as u64) as *mut OpusEncoder;
    if st.is_null() {
        if !error.is_null() {
    ...
    let ret = opus_encoder_init(st, Fs, channels, application);
    if !error.is_null() {
        *error = ret;
    }
    if ret != OPUS_OK {
...
pub unsafe fn compute_stereo_width(
    ...
    for i in (0..frame_size - 3).step_by(4) {
        let mut x = *pcm.offset((2 * i) as isize);
        let mut y = *pcm.offset((2 * i + 1) as isize);
        let mut pxx = x * x;
        let mut pxy = x * y;
        let mut pyy = y * y;
...
unsafe fn compute_silk_rate_for_hybrid(
    ...
    let N = (::core::mem::size_of::<[[i32; 5]; 7]>() as u64)
...
unsafe fn compute_frame_energy(pcm: &[f32], frame_size: i32, channels: i32, _arch: i32) -> f32 {
...
pub unsafe fn opus_encode_native<T>(
    ...
    let mut max_data_bytes = if 1276 < out_data_bytes {
        1276
    } else {
        out_data_bytes
    };
    ...
    let voice_est = if (*st).signal_type == OPUS_SIGNAL_VOICE {
        ...
        let mut voice_est = (*st).voice_ratio * 327 >> 8;
    ...
    if (*st).application == OPUS_APPLICATION_RESTRICTED_LOWDELAY {
        ...
        let mode_voice = ((1.0f32 - stereo_width) * mode_thresholds[0 as usize][0 as usize] as f32
            + stereo_width * mode_thresholds[1 as usize][0 as usize] as f32)
            as i32;
        let mode_music = ((1.0f32 - stereo_width) * mode_thresholds[1 as usize][1 as usize] as f32
    ...
    memcpy(
        ...
        &mut *((*st).delay_buffer)
            ...
            as *mut f32 as *const core::ffi::c_void,
    ...
    let hp_freq_smth1 = if (*st).mode == MODE_CELT_ONLY {
        ...
        (*(silk_enc as *mut silk_encoder)).state_Fxx[0 as usize]
            .sCmn
    ...
    if float_api != 0 {
        ...
        if !(sum < 1e9f32) || sum != sum {
            memset(
                &mut *pcm_buf
                    .as_mut_ptr()
                    .offset((total_buffer * (*st).channels) as isize) as *mut f32
                    as *mut core::ffi::c_void,
                0,
                ((frame_size * (*st).channels) as u64)
                    .wrapping_mul(::core::mem::size_of::<f32>() as u64),
            );
            (*st).hp_mem[3 as usize] = 0 as f32;
    ...
    if (*st).mode != MODE_CELT_ONLY {
        ...
        if !((*st).energy_masking).is_null() && (*st).use_vbr != 0 && (*st).lfe == 0 {
            ...
            for c in 0..(*st).channels {
                for i in 0..end {
                    let mut mask =
                        if (if *((*st).energy_masking).offset((21 * c + i) as isize) < 0.5f32 {
        ...
        if prefill != 0 {
            ...
            memset(
                ...
                0,
                (prefill_offset as u64).wrapping_mul(::core::mem::size_of::<f32>() as u64),
            );
    ...
    if (*st).mode != MODE_SILK_ONLY {
        let mut celt_pred: f32 = 2 as f32;
    ...
    if (*st).channels * ((*st).encoder_buffer - (frame_size + total_buffer)) > 0 {
        memmove(
            ...
            &mut *((*st).delay_buffer)
                .as_mut_ptr()
                .offset(((*st).channels * frame_size) as isize) as *mut f32
        ...
        memcpy(
            ...
            ) as *mut f32 as *mut core::ffi::c_void,
            &mut *pcm_buf.as_mut_ptr().offset(0 as isize) as *mut f32 as *const core::ffi::c_void,
...
pub unsafe fn opus_encoder_ctl_impl(st: *mut OpusEncoder, request: i32, args: VarArgs) -> i32 {
    ...
    match request {
        ...
        OPUS_RESET_STATE => {
            ...
            (*st).analysis = TonalityAnalysisState::new((*st).Fs);
            let start = &mut (*st).stream_channels as *mut i32 as *mut i8;
            memset(
