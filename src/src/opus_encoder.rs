...
impl OpusEncoder {
    pub fn new(Fs: i32, channels: i32, application: i32) -> Result<Self, i32> {
        ...
        let mut analysis = TonalityAnalysisState::new(Fs);
...
pub fn opus_encode_native<T>(
    ...
    let hp_freq_smth1 = if st.mode == MODE_CELT_ONLY {
        ...
        silk_enc.state_Fxx[0 as usize].sCmn.variable_HP_smth1_Q15
    };
                ...
                ((st.channels * st.Fs / 400) as u64)
                    ...
                    .wrapping_add(
                        (0 * tmp_prefill.as_mut_ptr().offset_from(
                            &mut *(st.delay_buffer).as_mut_ptr().offset(
                                ((st.encoder_buffer - total_buffer - st.Fs / 400) * st.channels)
                                    as isize,
                            ),
                        ) as i64) as u64,
                    ),
    ...
    if st.channels * (st.encoder_buffer - (frame_size + total_buffer)) > 0 {
        ...
        unsafe {
            memcpy(
                ...
                &mut *pcm_buf.as_mut_ptr().offset(0 as isize) as *mut f32
                    as *const core::ffi::c_void,
