...
pub fn silk_InitEncoder(arch: i32) -> (silk_encoder, silk_EncControlStruct) {
    let mut encState = silk_encoder::new(arch);
    encState.nChannelsAPI = 1;
    encState.nChannelsInternal = 1;
    let mut encStatus = silk_EncControlStruct {
        nChannelsAPI: 1,
        nChannelsInternal: 1,
        API_sampleRate: 0,
    ...
    silk_QueryEncoder(&encState, &mut encStatus);
    (encState, encStatus)
}

fn silk_QueryEncoder(psEnc: &silk_encoder, encStatus: &mut silk_EncControlStruct) {
    let state_Fxx = &psEnc.state_Fxx;
    ...
    encStatus.internalSampleRate = state_Fxx[0].sCmn.fs_kHz as i16 as i32 * 1000;
    encStatus.allowBandwidthSwitch = state_Fxx[0].sCmn.allow_bandwidth_switch;
    encStatus.inWBmodeWithoutVariableLP =
        (state_Fxx[0].sCmn.fs_kHz == 16 && state_Fxx[0].sCmn.sLP.mode == 0) as i32;
}
...
pub unsafe fn silk_Encode(
    ...
    if (*encControl).nChannelsInternal > psEnc.nChannelsInternal {
        psEnc.state_Fxx[1] = silk_encoder_state_FLP::new(psEnc.state_Fxx[0].sCmn.arch);
        memset(
    ...
    if prefillFlag != 0 {
        ...
        while n < (*encControl).nChannelsInternal {
            psEnc.state_Fxx[n as usize] =
                silk_encoder_state_FLP::new(psEnc.state_Fxx[n as usize].sCmn.arch);
            if prefillFlag == 2 {
    ...
    assert!(
        (*encControl).nChannelsInternal == 1
            || psEnc.state_Fxx[0 as usize].sCmn.fs_kHz == psEnc.state_Fxx[1 as usize].sCmn.fs_kHz
    );
    ...
    loop {
        ...
        if (*encControl).nChannelsAPI == 2 && (*encControl).nChannelsInternal == 2 {
            ...
            let range = (psEnc.state_Fxx[0].sCmn.inputBufIx + 2) as usize..;
            ret += silk_resampler(
                &mut (*(psEnc.state_Fxx).as_mut_ptr().offset(0 as isize))
                    .sCmn
                    .resampler_state,
                &mut (&mut psEnc.state_Fxx)[0].sCmn.inputBuf[range][..nSamplesToBuffer as usize],
            ...
            let range = (psEnc.state_Fxx[1].sCmn.inputBufIx + 2) as usize..;
            ret += silk_resampler(
                &mut (*(psEnc.state_Fxx).as_mut_ptr().offset(1 as isize))
                    .sCmn
                    .resampler_state,
                &mut (&mut psEnc.state_Fxx)[1].sCmn.inputBuf[range][..nSamplesToBuffer as usize],
                &buf[..nSamplesFromInput as usize],
            );
            ...
            if psEnc.nPrevChannelsInternal == 2 && psEnc.state_Fxx[0].sCmn.nFramesEncoded == 0 {
                ...
                ret += silk_resampler(
                    &mut (*(psEnc.state_Fxx).as_mut_ptr().offset(1 as isize))
                        .sCmn
                        .resampler_state,
                    &mut (&mut psEnc.state_Fxx)[1].sCmn.inputBuf[range],
                    &buf[..nSamplesFromInput as usize],
                );
                n = 0;
        ...
        if (*encControl).nChannelsInternal == 2 {
            silk_stereo_LR_to_MS(
                ...
                &mut *((*(psEnc.state_Fxx).as_mut_ptr().offset(1 as isize))
                    ...
                    .as_mut_ptr()
                    .offset(2 as isize),
                (psEnc.sStereo.predIx[psEnc.state_Fxx[0 as usize].sCmn.nFramesEncoded as usize])
                    .as_mut_ptr(),
                ...
                == 0
            {
                if psEnc.prev_decode_only_middle == 1 {
                    ...
                    psEnc.state_Fxx[1 as usize].sCmn.prevSignalType = TYPE_NO_VOICE_ACTIVITY as i8;
            ...
            memcpy(
                (psEnc.state_Fxx[0 as usize].sCmn.inputBuf).as_mut_ptr() as *mut core::ffi::c_void,
    ...
    (*encControl).inWBmodeWithoutVariableLP = (psEnc.state_Fxx[0 as usize].sCmn.fs_kHz == 16
        ...
        as i32;
    (*encControl).internalSampleRate = psEnc.state_Fxx[0 as usize].sCmn.fs_kHz as i16 as i32 * 1000;
