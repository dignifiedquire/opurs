...
pub fn silk_stereo_encode_pred(ps_range_enc: &mut EcEnc, ix: &[[i8; 3]]) {
    let mut n: i32;
    n = 5 * ix[0][2_usize] as i32 + ix[1][2_usize] as i32;
    debug_assert!(n < 25);
    ec_enc_icdf(ps_range_enc, n, &SILK_STEREO_PRED_JOINT_ICDF, 8);
    n = 0;
    while n < 2 {
...
pub fn silk_stereo_encode_mid_only(ps_range_enc: &mut EcEnc, mid_only_flag: i8) {
    ec_enc_icdf(
        ps_range_enc,
