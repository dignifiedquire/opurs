...
/// (_i.e., they include 2 history samples at index 0..1, frame at 2..frame_length+1).
...
pub fn silk_stereo_lr_to_ms(
    ...
    let is10ms_frame: i32 = (frame_length == 10 * fs_k_hz) as i32;
    smooth_coef_q16 = if is10ms_frame != 0 {
    ...
    total_rate_bps -= if is10ms_frame != 0 { 1200 } else { 600 };
    if total_rate_bps < 1 {
        total_rate_bps = 1;
    }
    let min_mid_rate_bps: i32 = 2000 + fs_k_hz as i16 as i32 * 600;
    ...
    if to_mono != 0 {
        ...
        pred_q13[0_usize] = 0;
        pred_q13[1_usize] = 0;
        silk_stereo_quant_pred(&mut pred_q13, &mut state.pred_ix[frame_idx]);
    ...
    if state.mid_only_flags[frame_idx] as i32 == 1 {
        state.silent_side_len = (state.silent_side_len as i32
            + (frame_length - STEREO_INTERP_LEN_MS as i32 * fs_k_hz))
            as i16;
        if (state.silent_side_len as i32) < LA_SHAPE_MS * fs_k_hz {
            state.mid_only_flags[frame_idx] = 0;
        } else {
    ...
    let denom_q16: i32 = ((1) << 16) / (8 * fs_k_hz);
    ...
    n = 0;
    while n < STEREO_INTERP_LEN_MS as i32 * fs_k_hz {
        ...
        // In the c code, x2 was pointing at frame start (offset 2 in input_buf).
        ...
        // In the c caller (enc_API.rs): x1 = &input_buf[2], x2 = &input_buf[2]
        // In c original: mid = x1 - 2 = &input_buf[0]
        // Output: x2[n-1] where n goes from 0..interp_len
        // But our x1 starts at input_buf[0] (caller passes &input_buf[0..frame_length+2])
        // so mid = x1 directly (no offset needed)
        // x2 also starts at input_buf[0], so output x2[n-1]... but n starts at 0, giving x2[-1]?
        //
        // Actually in the original, the caller passes &input_buf[2] for both x1 and x2.
        // mid = x1 - 2 = &input_buf[0], and mid[n] for n=0..frame_length+2 covers input_buf[0..frame_length+2]
        // x2[n-1] for n=0 gives x2[-1] = input_buf[1], which is the last history sample.
        //
        // For our safe version, we'll change the caller to pass &input_buf[0..frame_length+2]
        // for both x1 and x2, and adjust the x2 write index accordingly.
        // x2 output index: in c, x2.offset(n-1) = input_buf[2 + n - 1] = input_buf[n+1]
        // In our version with x2 starting at input_buf[0]: x2[n+1]
        x2[(n + 1) as usize] =
    ...
    n = STEREO_INTERP_LEN_MS as i32 * fs_k_hz;
    while n < frame_length {
        sum = (((x1[n as usize] as i32
            + x1[(n + 2) as usize] as i32
            + ((x1[(n + 1) as usize] as u32) << 1) as i32) as u32)
