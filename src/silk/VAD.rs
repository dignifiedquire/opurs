...
pub fn silk_VAD_Init(psSilk_VAD: &mut silk_VAD_state) -> i32 {
    ...
    *psSilk_VAD = Default::default();
    ...
    while b < VAD_N_BANDS {
        unsafe {
            *psSilk_VAD.NoiseLevelBias.get_unchecked_mut(b as usize) = silk_max_32(50 / (b + 1), 1);
        }
        b += 1;
    }
    b = 0;
    while b < VAD_N_BANDS {
        unsafe {
            *psSilk_VAD.NL.get_unchecked_mut(b as usize) =
                100 * *psSilk_VAD.NoiseLevelBias.get_unchecked(b as usize);
            *psSilk_VAD.inv_NL.get_unchecked_mut(b as usize) =
                0x7fffffff / *psSilk_VAD.NL.get_unchecked(b as usize);
        }
        b += 1;
    }
    ...
    while b < VAD_N_BANDS {
        unsafe {
            *psSilk_VAD.NrgRatioSmth_Q8.get_unchecked_mut(b as usize) = 100 * 256;
        }
        b += 1;
    }
    ret
...
pub fn silk_VAD_GetSA_Q8_c(psEncC: &mut silk_encoder_state, pIn: &[i16]) -> i32 {
    ...
    while i > 0 {
        unsafe {
            *X.get_unchecked_mut((i - 1) as usize) =
                (*X.get_unchecked((i - 1) as usize) as i32 >> 1) as i16;
            *X.get_unchecked_mut(i as usize) = (*X.get_unchecked(i as usize) as i32
                - *X.get_unchecked((i - 1) as usize) as i32)
                as i16;
        }
        i -= 1;
    }
    unsafe {
        *X.get_unchecked_mut(0) = (*X.get_unchecked(0) as i32 - psSilk_VAD.HPstate as i32) as i16;
    }
    ...
    while b < VAD_N_BANDS {
        ...
        unsafe {
            *Xnrg.get_unchecked_mut(b as usize) = *psSilk_VAD.XnrgSubfr.get_unchecked(b as usize);
        }
        s = 0;
        while s < VAD_INTERNAL_SUBFRAMES {
            {
                let start =
                    (unsafe { *X_offset.get_unchecked(b as usize) } + dec_subframe_offset) as usize;
                let end = start + dec_subframe_length as usize;
                sumSquared = silk_vad_energy(&X[start..end], psEncC.arch);
            }
            unsafe {
                if s < VAD_INTERNAL_SUBFRAMES - 1 {
                    *Xnrg.get_unchecked_mut(b as usize) =
                        if (*Xnrg.get_unchecked(b as usize) as u32).wrapping_add(sumSquared as u32)
                            & 0x80000000_u32
                            != 0
                        {
                            silk_int32_MAX
                    ...
                    *Xnrg.get_unchecked_mut(b as usize) = if (*Xnrg.get_unchecked(b as usize)
                        as u32)
        ...
        unsafe {
            *psSilk_VAD.XnrgSubfr.get_unchecked_mut(b as usize) = sumSquared;
        }
        b += 1;
    }
            ...
            if speech_nrg > 0 {
                if *Xnrg.get_unchecked(b as usize) as u32 & 0xff800000_u32 == 0 {
                    *NrgToNoiseRatio_Q8.get_unchecked_mut(b as usize) =
                        ((*Xnrg.get_unchecked(b as usize) as u32) << 8) as i32
                            / (*psSilk_VAD.NL.get_unchecked(b as usize) + 1);
                } else {
                    *NrgToNoiseRatio_Q8.get_unchecked_mut(b as usize) = *Xnrg
                        .get_unchecked(b as usize)
                        / ((*psSilk_VAD.NL.get_unchecked(b as usize) >> 8) + 1);
                }
                ...
                input_tilt = (input_tilt as i64
                    + ((*TILT_WEIGHTS.get_unchecked(b as usize) as i64 * SNR_Q7 as i16 as i64)
                        >> 16)) as i32;
            } else {
    ...
    while b < VAD_N_BANDS {
        speech_nrg += (b + 1)
            * ((unsafe { *Xnrg.get_unchecked(b as usize) }
                - unsafe { *psSilk_VAD.NL.get_unchecked(b as usize) })
                >> 4);
        b += 1;
    }
    ...
    while b < VAD_N_BANDS {
        unsafe {
            *psSilk_VAD.NrgRatioSmth_Q8.get_unchecked_mut(b as usize) =
                (*psSilk_VAD.NrgRatioSmth_Q8.get_unchecked(b as usize) as i64
                    + (((*NrgToNoiseRatio_Q8.get_unchecked(b as usize)
                        - *psSilk_VAD.NrgRatioSmth_Q8.get_unchecked(b as usize))
                        as i64
                        * smooth_coef_Q16 as i16 as i64)
                        >> 16)) as i32;
            SNR_Q7 =
                3 * (silk_lin2log(*psSilk_VAD.NrgRatioSmth_Q8.get_unchecked(b as usize)) - 8 * 128);
            *psEncC.input_quality_bands_Q15.get_unchecked_mut(b as usize) =
                silk_sigm_Q15((SNR_Q7 - 16 * 128) >> 4);
        }
        b += 1;
    }
    ret
...
fn silk_VAD_GetNoiseLevels(pX: &[i32; 4], psSilk_VAD: &mut silk_VAD_state) {
    ...
    while k < VAD_N_BANDS {
        unsafe {
            ...
            nrg = if (*pX.get_unchecked(k as usize) as u32)
                .wrapping_add(*psSilk_VAD.NoiseLevelBias.get_unchecked(k as usize) as u32)
                & 0x80000000_u32
                != 0
            {
                silk_int32_MAX
            ...
            *psSilk_VAD.inv_NL.get_unchecked_mut(k as usize) =
                (*psSilk_VAD.inv_NL.get_unchecked(k as usize) as i64
                    + (((inv_nrg - *psSilk_VAD.inv_NL.get_unchecked(k as usize)) as i64
                        * coef as i16 as i64)
                        >> 16)) as i32;
