...
pub fn silk_NLSF_del_dec_quant(
    ...
    nStates = 1;
    unsafe { *RD_Q25.get_unchecked_mut(0_usize) = 0; }
    unsafe { *prev_out_Q10.get_unchecked_mut(0_usize) = 0; }
    i = order as i32 - 1;
    while i >= 0 {
        ...
        while j < nStates {
            pred_Q10 =
                (unsafe { *pred_coef_Q8.get_unchecked(i as usize) } as i16 as i32 * unsafe { *prev_out_Q10.get_unchecked(j as usize) } as i32) >> 8;
            ...
            unsafe { *ind.get_unchecked_mut(j as usize).get_unchecked_mut(i as usize) = ind_tmp as i8; }
            out0_Q10 = unsafe { *out0_Q10_table.get_unchecked((ind_tmp + NLSF_QUANT_MAX_AMPLITUDE_EXT) as usize) } as i16;
            out1_Q10 = unsafe { *out1_Q10_table.get_unchecked((ind_tmp + NLSF_QUANT_MAX_AMPLITUDE_EXT) as usize) } as i16;
            out0_Q10 = (out0_Q10 as i32 + pred_Q10) as i16;
            out1_Q10 = (out1_Q10 as i32 + pred_Q10) as i16;
            unsafe { *prev_out_Q10.get_unchecked_mut(j as usize) = out0_Q10; }
            unsafe { *prev_out_Q10.get_unchecked_mut((j + nStates) as usize) = out1_Q10; }
            if ind_tmp + 1 >= NLSF_QUANT_MAX_AMPLITUDE {
                if ind_tmp + 1 == NLSF_QUANT_MAX_AMPLITUDE {
                    rate0_Q5 = unsafe { *rates_Q5.get_unchecked((ind_tmp + NLSF_QUANT_MAX_AMPLITUDE) as usize) } as i32;
                    rate1_Q5 = 280;
                } else {
                ...
                if ind_tmp == -NLSF_QUANT_MAX_AMPLITUDE {
                    rate0_Q5 = 280;
                    rate1_Q5 = unsafe { *rates_Q5.get_unchecked((ind_tmp + 1 + NLSF_QUANT_MAX_AMPLITUDE) as usize) } as i32;
                } else {
            ...
            unsafe { *RD_Q25.get_unchecked_mut(j as usize) = RD_tmp_Q25
                + diff_Q10 as i16 as i32 * diff_Q10 as i16 as i32 * *w_Q5.get_unchecked(i as usize) as i32
                + mu_Q20 as i16 as i32 * rate0_Q5 as i16 as i32; }
            diff_Q10 = in_Q10 - out1_Q10 as i32;
            unsafe { *RD_Q25.get_unchecked_mut((j + nStates) as usize) = RD_tmp_Q25
                + diff_Q10 as i16 as i32 * diff_Q10 as i16 as i32 * *w_Q5.get_unchecked(i as usize) as i32
        ...
        if nStates <= NLSF_QUANT_DEL_DEC_STATES / 2 {
            j = 0;
            while j < nStates {
                unsafe { *ind.get_unchecked_mut((j + nStates) as usize).get_unchecked_mut(i as usize) =
            ...
            while j < NLSF_QUANT_DEL_DEC_STATES {
                unsafe { *ind.get_unchecked_mut(j as usize).get_unchecked_mut(i as usize) =
                    *ind.get_unchecked((j - nStates) as usize).get_unchecked(i as usize); }
                j += 1;
            }
        } else {
            j = 0;
            while j < NLSF_QUANT_DEL_DEC_STATES {
                unsafe {
                if *RD_Q25.get_unchecked(j as usize) > *RD_Q25.get_unchecked((j + NLSF_QUANT_DEL_DEC_STATES) as usize) {
                    *RD_max_Q25.get_unchecked_mut(j as usize) = *RD_Q25.get_unchecked(j as usize);
                    *RD_min_Q25.get_unchecked_mut(j as usize) = *RD_Q25.get_unchecked((j + NLSF_QUANT_DEL_DEC_STATES) as usize);
                    *RD_Q25.get_unchecked_mut(j as usize) = *RD_min_Q25.get_unchecked(j as usize);
                    *RD_Q25.get_unchecked_mut((j + NLSF_QUANT_DEL_DEC_STATES) as usize) = *RD_max_Q25.get_unchecked(j as usize);
                    ...
                    *prev_out_Q10.get_unchecked_mut((j + NLSF_QUANT_DEL_DEC_STATES) as usize) = out0_Q10;
                    *ind_sort.get_unchecked_mut(j as usize) = j + NLSF_QUANT_DEL_DEC_STATES;
                } else {
                    *RD_min_Q25.get_unchecked_mut(j as usize) = *RD_Q25.get_unchecked(j as usize);
                    *RD_max_Q25.get_unchecked_mut(j as usize) = *RD_Q25.get_unchecked((j + NLSF_QUANT_DEL_DEC_STATES) as usize);
            ...
            loop {
                ...
                *prev_out_Q10.get_unchecked_mut(ind_max_min as usize) =
                    *prev_out_Q10.get_unchecked((ind_min_max + NLSF_QUANT_DEL_DEC_STATES) as usize);
            ...
            while j < NLSF_QUANT_DEL_DEC_STATES {
                unsafe {
                *ind.get_unchecked_mut(j as usize).get_unchecked_mut(i as usize) =
                    (*ind.get_unchecked(j as usize).get_unchecked(i as usize) as i32 + (*ind_sort.get_unchecked(j as usize) >> 2)) as i8;
                }
                j += 1;
            }
        }
    ...
    while j < order as i32 {
        unsafe {
        *indices.get_unchecked_mut(j as usize) = *ind.get_unchecked((ind_tmp & (NLSF_QUANT_DEL_DEC_STATES - 1)) as usize).get_unchecked(j as usize);
        }
        j += 1;
    }
    unsafe { *indices.get_unchecked_mut(0) = (*indices.get_unchecked(0) as i32 + (ind_tmp >> 2)) as i8; }
    min_Q25
}
