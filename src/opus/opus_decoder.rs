...
fn opus_decode_frame(
    ...
    let mut pcm_transition_celt = [0.0f32; MAX_F5_CH];
    ...
    let mut pcm_silk = [0.0f32; MAX_F10_CH];
    if mode != MODE_CELT_ONLY {
    ...
    debug_assert!(vla_1 <= MAX_F5_CH);
    let mut pcm_transition_silk = [0.0f32; MAX_F5_CH];
    if transition != 0 && mode != MODE_CELT_ONLY {
        opus_decode_frame(
            st,
            None,
    ...
    if transition != 0 {
        let pcm_transition: &[f32] = if mode == MODE_CELT_ONLY {
            &pcm_transition_celt
        } else {
            &pcm_transition_silk
        };
    ...
    if st.decode_gain != 0 {
        let gain: f32 = celt_exp2(6.488_141e-4_f32 * st.decode_gain as f32);
        i = 0;
