...
pub struct OpusCustomEncoder {
    ...
    // TODO: use Box<[T]> for these below
    // size = channels * mode.overlap
    pub in_mem: Vec<celt_sig>,
    // size = channels * COMBFILTER_MAXPERIOD
    pub prefilter_mem: Vec<celt_sig>,
    // size = channels * mode.nbEBands
    pub oldBandE: Vec<opus_val16>,
    // size = channels * mode.nbEBands
    pub oldLogE: Vec<opus_val16>,
    // size = channels * mode.nbEBands
    pub oldLogE2: Vec<opus_val16>,
    // size = channels * mode.nbEBands
    pub energyError: Vec<opus_val16>,
}

impl OpusCustomEncoder {
    // mirrors `celt_encoder_init_arch`
    pub fn new_arch(mode: &'static OpusCustomMode, channels: i32, arch: i32) -> Result<Self, i32> {
        ...
        let mut res = Self {
            ...
            stereo_saving: 0.,
            intensity: 0,
            energy_mask: std::ptr::null_mut(),
            spec_avg: 0.,
            in_mem: vec![0.; channels as usize * mode.overlap],
            prefilter_mem: vec![0.; (channels * COMBFILTER_MAXPERIOD) as usize],
            oldBandE: vec![0.; channels as usize * mode.nbEBands],
            oldLogE: vec![0.; channels as usize * mode.nbEBands],
            oldLogE2: vec![0.; channels as usize * mode.nbEBands],
            energyError: vec![0.; channels as usize * mode.nbEBands],
        };

        opus_custom_encoder_ctl!(&mut res, OPUS_RESET_STATE);
    ...
    pub fn new(sampling_rate: i32, channels: i32, arch: i32) -> Result<Self, i32> {
        ...
        let mut res = Self::new_arch(mode, channels, arch)?;
        res.upsample = resampling_factor(sampling_rate);
        Ok(res)
    }
}
...
unsafe fn run_prefilter(
    ...
    debug_assert!(CC <= 2, "CC: {CC} too large");
    ...
    let vla = (CC * (N + COMBFILTER_MAXPERIOD)) as usize;
    let mut _pre: Vec<celt_sig> = vec![0.; vla];

    let mid = (N + COMBFILTER_MAXPERIOD) as usize;
    let (a, b) = _pre.split_at_mut(mid);
    let mut pre = [a, b];

    for c in 0..CC {
        //  OPUS_COPY(pre[c], prefilter_mem+c*COMBFILTER_MAXPERIOD, COMBFILTER_MAXPERIOD);
        let start = (c * COMBFILTER_MAXPERIOD) as usize;
        let end = start + COMBFILTER_MAXPERIOD as usize;

        pre[c as usize][..COMBFILTER_MAXPERIOD as usize]
            .copy_from_slice(&st.prefilter_mem[start..end]);

        // OPUS_COPY(pre[c]+COMBFILTER_MAXPERIOD, in+c*(N+overlap)+overlap, N);
        let len = N as usize;
        let period = COMBFILTER_MAXPERIOD as usize;
        let start = (c * (N + overlap) + overlap) as usize;
        let end = start + len;

        pre[c as usize][period..period + len].copy_from_slice(&in_0[start..end])
    }
    ...
    if st.analysis.valid != 0 {
        gain1 = gain1 * st.analysis.max_pitch_ratio;
    }
    pf_threshold = 0.2f32;
    ...
    for c in 0..CC {
        ...
        let len = overlap as usize;
        let start_in_0 = (c * (N + overlap)) as usize;
        let start = (c * overlap) as usize;
        in_0[start_in_0..start_in_0 + len].copy_from_slice(&st.in_mem[start..start + len]);

        if offset != 0 {
            comb_filter(
                in_0[(c * (N + overlap)) as usize + overlap as usize..].as_mut_ptr(),
                (pre[c as usize].as_mut_ptr()).offset(COMBFILTER_MAXPERIOD as isize),
                st.prefilter_period,
                st.prefilter_period,
                offset,
                -st.prefilter_gain,
        ...
        comb_filter(
            in_0[(c * (N + overlap)) as usize + overlap as usize + offset as usize..].as_mut_ptr(),
            (pre[c as usize].as_mut_ptr())
        ...
        let start = (c * (N + overlap) + N) as usize;
        let end = start + overlap as usize;
        let src = &in_0[start..end];
        let start = (c * overlap) as usize;
        let end = start + overlap as usize;
        st.in_mem[start..end].copy_from_slice(src);

        if N > COMBFILTER_MAXPERIOD {
            // OPUS_COPY(prefilter_mem+c*COMBFILTER_MAXPERIOD, pre[c]+N, COMBFILTER_MAXPERIOD);
            let start = (c * COMBFILTER_MAXPERIOD) as usize;
            let end = start + COMBFILTER_MAXPERIOD as usize;
            st.prefilter_mem[start..end]
                .copy_from_slice(&pre[c as usize][N as usize..(N + COMBFILTER_MAXPERIOD) as usize]);
        } else {
            // OPUS_MOVE(prefilter_mem+c*COMBFILTER_MAXPERIOD, prefilter_mem+c*COMBFILTER_MAXPERIOD+N, COMBFILTER_MAXPERIOD-N);
            let start = (c * COMBFILTER_MAXPERIOD + N) as usize;
            let end = start + (COMBFILTER_MAXPERIOD - N) as usize;
            let dst = (c * COMBFILTER_MAXPERIOD) as usize;
            st.prefilter_mem.copy_within(start..end, dst);

            // OPUS_COPY(prefilter_mem+c*COMBFILTER_MAXPERIOD+COMBFILTER_MAXPERIOD-N, pre[c]+COMBFILTER_MAXPERIOD, N);
            let start = (c * COMBFILTER_MAXPERIOD + COMBFILTER_MAXPERIOD - N) as usize;
            let end = start + N as usize;
            st.prefilter_mem[start..end].copy_from_slice(
                &pre[c as usize]
                    [COMBFILTER_MAXPERIOD as usize..(COMBFILTER_MAXPERIOD + N) as usize],
            );
        }
    }
    *gain = gain1;
...
pub unsafe fn celt_encode_with_ec(
    ...
    let enc = if let Some(enc) = enc {
        ...
        #[cfg(feature = "ent-dump")]
        let s = hex::encode(&compressed_slice);
        _enc = ec_enc_init(compressed_slice);
        #[cfg(feature = "ent-dump")]
        eprintln!("init ec_enc {}: {}\n{:?}", nbCompressedBytes, s, _enc,);
    ...
    sample_max = if st.overlap_max > celt_maxabs16(pcm, (C * (N - overlap) / st.upsample) as usize)
    {
        st.overlap_max
    } else {
    ...
    loop {
        i = start;
        while i < end {
            if (bandLogE[(i + c * nbEBands) as usize] - st.oldBandE[(i + c * nbEBands) as usize])
                .abs()
                < 2.0f32
            {
                let ref mut fresh4 = *bandLogE.as_mut_ptr().offset((i + c * nbEBands) as isize);
                *fresh4 -= st.energyError[(i + c * nbEBands) as usize] * 0.25f32;
            }
            i += 1;
        }
        c += 1;
    ...
    loop {
        i = start;
        while i < end {
            st.energyError[(i + c * nbEBands) as usize] = if -0.5f32
    ...
    if CC == 2 && C == 1 {
        ...
        st.oldBandE
            .copy_within(0..(nbEBands as usize), nbEBands as usize);
    }
    if isTransient == 0 {
        // OPUS_COPY(oldLogE2, oldLogE, CC*nbEBands);
        let end = (CC * nbEBands) as usize;
        st.oldLogE2[..end].copy_from_slice(&st.oldLogE[..end]);
        // OPUS_COPY(oldLogE, oldBandE, CC*nbEBands);
        st.oldLogE[..end].copy_from_slice(&st.oldBandE[..end]);
    } else {
        for i in 0..CC * nbEBands {
            st.oldLogE[i as usize] = if st.oldLogE[i as usize] < st.oldBandE[i as usize] {
                st.oldLogE[i as usize]
            } else {
                st.oldBandE[i as usize]
            };
        }
    }
    c = 0;
    loop {
        for i in 0..start {
            st.oldBandE[(c * nbEBands + i) as usize] = 0.;
            let ref mut fresh5 = st.oldLogE2[(c * nbEBands + i) as usize];
            *fresh5 = -28.0f32;
            st.oldLogE[(c * nbEBands + i) as usize] = *fresh5;
        }
        for i in end..nbEBands {
            st.oldBandE[(c * nbEBands + i) as usize] = 0.;
            let ref mut fresh6 = st.oldLogE2[(c * nbEBands + i) as usize];
            *fresh6 = -28.0f32;
            st.oldLogE[(c * nbEBands + i) as usize] = *fresh6;
        }
        c += 1;
        if !(c < CC) {
            break;
