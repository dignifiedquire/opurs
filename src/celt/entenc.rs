...
fn ec_write_byte(this: &mut ec_enc, value: u32) -> i32 {
    ...
    unsafe { *this.buf.get_unchecked_mut(this.offs as usize) = value as u8; }
    this.offs += 1;
...
fn ec_write_byte_at_end(this: &mut ec_enc, value: u32) -> i32 {
    ...
    this.end_offs += 1;
    unsafe { *this.buf.get_unchecked_mut((this.storage - this.end_offs) as usize) = value as u8; }
...
pub fn ec_enc_icdf(this: &mut ec_enc, s: i32, icdf: &[u8], ftb: u32) {
    ...
    if s > 0 {
        this.val = this.val.wrapping_add(
            this.rng
                .wrapping_sub(r.wrapping_mul(unsafe { *icdf.get_unchecked(s as usize - 1) } as u32)),
        );
        this.rng = r.wrapping_mul((unsafe { *icdf.get_unchecked(s as usize - 1) } as i32 - unsafe { *icdf.get_unchecked(s as usize) } as i32) as u32);
    } else {
        this.rng = this
            .rng
